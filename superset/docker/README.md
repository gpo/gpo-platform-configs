...
I hosted it at `mwelke/apache-superset-mysql-pg:5.0.0`. It's public because there are no secrets in it - this is just a few pip packages added. We can use this right now, or we can host this image ourselves. We could use Docker Hub for its free tier, or we could use Artifact Registry in GCP to keep things contained within GCP (but AR has no free tier so we'd start paying right away).
