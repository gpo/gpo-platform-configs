...
Stage and prod ship as two PRs, and the apply happens while each PR is open: stage PR → apply stage → merge → prod PR → apply prod → merge. `singletons` and `bootstrap` have no stage tier — put the plan in the PR and get a human to approve the apply. Full rules: [pull-requests.md](pull-requests.md).
